...
left_circ <- circular(left_angles, units = "degrees", type = "angles")
right_circ <- circular(right_angles, units = "degrees", type = "angles")
back_circ <- circular(back_angles, units = "degrees", type = "angles")
...
# Combine data
all_angles <- c(left_angles, right_angles, back_angles)
...
### WITH CONTINUOUS DATA (E.G., LVN)
lvn_angles <- sides[!is.na(LVN)][, .(BurialOrientationMean, LVN)]
lvn <- lvn_angles$LVN
lvn_circ <- circular(lvn_angles$BurialOrientationMean, type = "angles", units = "degrees", rotation = "clock")

# Convert angles from degrees to radians
angles_rad <- rad(lvn_circ) #angles_deg * pi / 180

# Compute circular-linear correlation (Mardia's coefficient)
circ_lin_corr <- circ.cor(angles_rad, lvn)

# Print the result
print(paste("Circular-linear correlation (Mardia's r):", round(circ_lin_corr, 4)))

# Define similarity index as absolute value
similarity_index <- abs(circ_lin_corr)
print(paste("Similarity index (|r|):", round(similarity_index, 4)))

# Fit Bayesian circular regression model (predicting angle from LVN)
lvn_angles$BurialOrientationMeanRad <- as.numeric(angles_rad)
model <- bpnr(pred.I = BurialOrientationMeanRad ~ LVN, data = lvn_angles, its = 1000, burn = 100, n.lag = 1)
...
I_pred <- beta0_I + beta1_I * lvn_angles$LVN
II_pred <- beta0_II + beta1_II * lvn_angles$LVN

# Compute predicted mean direction (arctangent)
fitted_angles <- atan2(I_pred, II_pred)

# Compute raw residuals
residuals_raw <- lvn_angles$BurialOrientationMeanRad - fitted_angles

# Wrap to [0, 2π)
residuals_wrapped <- residuals_raw %% (2 * pi)

# Center to (-π, π]
residuals_centered <- ifelse(residuals_wrapped > pi, residuals_wrapped - 2 * pi, residuals_wrapped)

# Add to dataframe
lvn_angles$residuals <- residuals_centered
lvn_angles$fitted_angles <- fitted_angles

library(ggplot2)

# Residuals vs. Fitted Angles
ggplot(lvn_angles, aes(x = fitted_angles, y = residuals)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Circular Residuals vs Fitted Angles",
       x = "Fitted Mean Direction (radians)",
       y = "Residual (radians)") +
  theme_minimal()

# Residuals vs. LVN predictor
ggplot(lvn_angles, aes(x = LVN, y = residuals)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Circular Residuals vs LVN",
       x = "LVN",
       y = "Residual (radians)") +
  theme_minimal()

# Histogram
ggplot(lvn_angles, aes(x = residuals)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black") +
  labs(title = "Histogram of Circular Residuals",
       x = "Residual (radians)",
       y = "Count") +
  theme_minimal()

# Rose diagram
residuals_circ <- circular(lvn_angles$residuals, type = "angles", units = "degrees", rotation = "clock")
rose.diag(residuals_circ, bins = 30, main = "Rose Diagram of Residuals")
