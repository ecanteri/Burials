...
ggplot(data = dat.tf, aes(factor(x = pna), y = log(Right + 1))) +
  geom_boxplot(colour = "skyblue2", outlier.shape = 21,
               outlier.colour = "skyblue2") +
  ylab("log (Right)\n") + # \n creates a space after the title
...
aic_table <- aic_table[gtools::mixedorder(aic_table$Model),]
...
## Based on the AICc scores and the drop test, it seems we have an effect of Culture and YearBP as fixed effect.
...
(p1 + p2 + plot_layout(axis_titles = "collect"))/(p3 + p4 + plot_layout(axis_titles = "collect"))/p5 + plot_annotation(tag_levels = "A", title = "Right burial side")
